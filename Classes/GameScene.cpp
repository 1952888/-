...
void GameScene::actionEndCallBack(Node* node)//删除精灵函数
{
	...
	map[spr->getRow()][spr->getCol()] = nullptr;
...
void GameScene::fillSprite()//填充精灵，是先让已存在的精灵下落，之后在创建新的精灵
{
	...
	for (int c = 0; c < COLS; ++c)//遍历map
	{
		...
		{
			spr = map[r][c];
			if (spr == nullptr)//如果这个位置没有精灵，变量自增
			{
				++removedSpriteOfCol;
			}
...
void GameScene::onTouchMoved(Touch *touch, Event *unused)
{
	...
	if (isUse)
	{
		...
		if (upRect.containsPoint(location))
		{
			...
			startSprite->setDisplayMode(DISPLAY_MODE_VERTICAL);
			markRemove(startSprite);
			removeSprite();
			isUse = false;
		...
		if (leftRect.containsPoint(location))
		{
			...
			startSprite->setDisplayMode(DISPLAY_MODE_HORIZONTAL);
			markRemove(startSprite);
			removeSprite();
